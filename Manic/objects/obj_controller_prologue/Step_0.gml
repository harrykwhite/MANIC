...
if (!global.game_pause) && (instance_exists(obj_player)){
	...
	if (deer_can_spawn){
		if (deer_spawn_time > 0){
			...
			var xx = random_range(camx - 50, camx + camw + 50);
			var yy = random_range(camy - 50, camy + camh + 50);
		
			while (onscreen(xx, yy, 128) || collision_rectangle(xx - 15, yy - 15, xx + 15, yy + 15, obj_p_solid, false, true)){
