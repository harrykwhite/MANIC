...
if (scr_input_is_pressed(InputBinding.Attack)) && (global.player_stamina_active) && (!global.game_pause){
    if (attack_can){
		...
		s = instance_create(global.player_position_x + lengthdir_x(lenpos, mdir), global.player_position_y + lengthdir_y(lenpos, mdir), obj_proj_2);
	    s.image_angle = mdir;
	    s.depth = depth + 1;
		s.owner_inst = obj_player.id;
		s.owner_offset_x = s.x - s.owner_inst.x;
