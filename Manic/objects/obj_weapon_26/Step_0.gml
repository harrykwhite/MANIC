...
if (!global.game_pause) && (global.weapon_slotammo[global.weapon_slotcurrent] > 0) && (global.player_stamina_active){
    if (scr_input_is_down(InputBinding.Attack)) && (shoot_can){
		...
		scr_player_flash(5);
		scr_player_flashlight_flash(1.18);
		...
		repeat(choose(2, 3)){
		    var shoot = instance_create(xpos + lengthdir_x(random_range(-2, 2), up(mdir)), ypos + lengthdir_y(random_range(-2, 2), up(mdir)), obj_proj_0);
