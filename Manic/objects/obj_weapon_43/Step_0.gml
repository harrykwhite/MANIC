...
if (scr_input_is_down(InputBinding.Attack)) && (global.player_stamina_active) && (!global.game_pause) && (global.weapon_slotammo[global.weapon_slotcurrent] > 0){
    if (shoot_can){
        var xpos = x + lengthdir_x(29, mdir) + lengthdir_x(3, up(mdir));
        var ypos = y + lengthdir_y(29, mdir) + lengthdir_y(3, up(mdir));
	...
	if (scr_input_is_released(InputBinding.Attack)){
		...
		//scr_sound_play(choose(snd_weapon_shell_0, snd_weapon_shell_1, snd_weapon_shell_2), false, 0.9, 1.2);
	}
}
